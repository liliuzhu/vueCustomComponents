...
// 针对数组内的时间进行排序
// 目前只有2个
export function sortTime(ary) {
  let begin = getTime(ary[0]);
  let end = getTime(ary[1]);
...
// 简单的判断是否在时间段内
// 数组里面只有2个元素
export function isBetweenRange(value , ary) {
  value = getTime(value)
  if (ary[0] === ary[1]) {
    return false;
  }
  let rangeBegin = getTime(ary[0])
  let rangeEnd = getTime(ary[1])

  const begin = rangeBegin > rangeEnd ? rangeEnd : rangeBegin
  const end = rangeBegin > rangeEnd ? rangeBegin : rangeEnd

  let isGte = value > begin;
  let isLte = value < end;
